// ads.js
import { db, doc, onSnapshot, collection, query, where, updateDoc, getDoc } from './firebase.js';
...
class AdEngine {
    ...
    initGoogleAds() {
        // If Ultra, never load Google Ads
        if (this.userTier === 'ultra') return;
        
        // If they already skipped this session, don't load
        if (sessionStorage.getItem('google_ads_disabled')) return;

        // Inject Google AdSense Script
        const script = document.createElement('script');
        script.async = true;
        script.src = "https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-7318663619327346";
        script.crossOrigin = "anonymous";
        document.head.appendChild(script);
        
        // Show Global Skip Button if Pro or has credits
        this.renderGlobalSkipButton();
    }

    renderGlobalSkipButton() {
        if (this.userTier === 'ultra') return;
        
        if (document.getElementById('global-skip-ads-btn')) return;

        const btn = document.createElement('button');
        btn.id = 'global-skip-ads-btn';
        btn.innerHTML = 'Skip Ads';
        btn.style.cssText = `
            position: fixed; bottom: 80px; right: 20px; z-index: 99999;
            background: linear-gradient(135deg, #ef4444, #ff3366);
            color: white; border: none; padding: 12px 24px; border-radius: 30px;
            font-weight: bold; cursor: pointer; box-shadow: 0 4px 15px rgba(255,51,102,0.4);
            font-family: 'Outfit', sans-serif; transition: transform 0.2s;
        `;
        ...
        btn.onclick = async () => {
            if (this.userTier === 'pro') {
                this.disableGoogleAds();
            } else if (this.userCredits > 0) {
                if(confirm('Use 1 Ad Credit to disable ads for this session?')) {
                    try {
                        const userRef = doc(db, 'users', this.userUid);
                        await updateDoc(userRef, { credits: this.userCredits - 1 });
                        this.disableGoogleAds();
                    } catch(e) {
                ...
                alert('You need Ad Credits or a Pro/Ultra subscription to skip ads!');
    ...
    disableGoogleAds() {
        sessionStorage.setItem('google_ads_disabled', 'true');
        // Remove script
        document.querySelectorAll('script[src*="adsbygoogle"]').forEach(s => s.remove());
        // Remove injected iframes/ins
        document.querySelectorAll('ins.adsbygoogle, iframe[name^="google_ads"]').forEach(el => el.remove());
        
        const btn = document.getElementById('global-skip-ads-btn');
        if (btn) btn.remove();
        alert('Ads disabled for this session.');
    }
    ...
    listenToCredits() {
        ...
        onSnapshot(doc(db, 'users', this.userUid), (snap) => {
            if (snap.exists()) {
                ...
                this.userCredits = data.credits || data.adCredits || 0;
                this.userTier = data.subscription || 'free';
                
                if (this.userTier === 'ultra') {
                    this.disableGoogleAds(); // immediately clear if ultra
                } else {
                    this.initGoogleAds();
                }
    ...
    listenToAds() {
        const q = query(collection(db, 'sponsors'), where('isActive', '==', true));
