$models = @(
    ...
    @{ cat='forestry_engineering'; files=@(
        @{name='forestry_feller_buncher_head.js'; func='createFellerBuncherHead'; desc='Feller Buncher Harvesting Head'},
        @{name='forestry_log_skidder_grapple.js'; func='createLogSkidderGrapple'; desc='Log Skidder Grapple'},
        @{name='forestry_mulcher_drum.js'; func='createForestryMulcherDrum'; desc='Forestry Mulcher Drum'},
        @{name='forestry_portable_sawmill.js'; func='createPortableSawmillBandsaw'; desc='Portable Sawmill Bandsaw'},
        @{name='forestry_stump_grinder.js'; func='createStumpGrinderWheel'; desc='Stump Grinder Wheel'}
    )}
)

foreach ($domain in $models) {
    foreach ($f in $domain.files) {
        $funcName = $f.func
        $content = @"
...
"@
        $path = "machines\$($f.name)"
        Set-Content -Path $path -Value $content -Encoding UTF8
        Write-Host "Created: $($f.name)"
    }
}
