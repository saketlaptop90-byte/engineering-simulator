...
async function loadMachine(machineEntry) {
  ...
  state.selectedPartIndex = -1;
    ...
    const isLearningMode = document.querySelector('[data-tab="learning"]').classList.contains('active');
    
    // Inject mock learning data (Sub-Phase 6A)
    const difficulties = ['🟢 Beginner', '🟡 Intermediate', '🔴 Advanced'];
    const diff = difficulties[Math.floor(Math.random() * difficulties.length)];
    
    document.getElementById('learn-title').innerText = machineEntry.name;
    document.getElementById('learn-difficulty').innerText = diff;
    document.getElementById('learn-what').innerText = "This is a detailed 3D simulation of " + machineEntry.name + ".";
    document.getElementById('learn-how').innerText = "It operates using principles of " + machineEntry.category.replace(/_/g, ' ') + ". Components interact dynamically to transfer energy or process inputs.";
    document.getElementById('learn-apps').innerText = "Used heavily in modern industry, research, and educational simulations.";
    document.getElementById('learn-proscons').innerText = "Pros: Highly efficient. Cons: Complex to manufacture.";
