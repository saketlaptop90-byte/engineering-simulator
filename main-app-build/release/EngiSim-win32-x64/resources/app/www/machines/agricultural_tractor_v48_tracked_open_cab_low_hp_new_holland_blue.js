...
export function createMachine(THREE) {
    ...
    // Core Materials
    const jdGreen = new THREE.MeshStandardMaterial({ color: 0x0033A0, metalness: 0.5, roughness: 0.3 });
    ...
    function buildEngineSystem() {
        ...
        // Engine Block
        const blockGeo = new THREE.BoxGeometry(0.6, 0.8, 1.0);
    ...
    function buildWheels() {
        const sys = new THREE.Group();
        const rearGeo = new THREE.CylinderGeometry(0.8, 0.8, 0.8, 32).rotateX(Math.PI/2);
        const rr = new THREE.Mesh(rearGeo, rubber);
        rr.position.set(1.2, 1.0, -1.2);
        const rl = rr.clone();
        rl.position.set(-1.2, 1.0, -1.2);
        
        const frontGeo = new THREE.CylinderGeometry(0.5, 0.5, 0.8, 32).rotateX(Math.PI/2);
