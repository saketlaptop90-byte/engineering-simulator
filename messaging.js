import { db, collection, addDoc, onSnapshot, query, orderBy, serverTimestamp, doc, getDoc, getDocs, setDoc, updateDoc, where, storage, ref, uploadBytesResumable, getDownloadURL } from './firebase.js';
...
export function initMessaging() {
    ...
    document.addEventListener('engisim-auth-changed', (e) => {
        ...
        if (user) {
            ...
            if (blocks && (blocks.global || blocks.chat)) {
                ...
                // Show ultra features
                const ultraDiv = document.getElementById('ultra-features');
                if (ultraDiv) {
                    if (e.detail.subscription === 'ultra') {
    ...
    btnSearch.onclick = async () => {
        ...
        if (!targetId) {
            targetId = window.prompt("Enter the Unique ID of the user you want to chat with (e.g. @bob):");
            if (!targetId) return;
        ...
        try {
            ...
            alert("Error creating chat: " + (err.message || err.toString()));
        }
    };
    ...
    if (fileUpload) {
        fileUpload.onchange = async (e) => {
            ...
            const user = getCurrentUser();
            const uniqueId = getCurrentUniqueId();
            const fileRef = ref(storage, `chat_media/${currentChatId}/${Date.now()}_${file.name}`);
            
            const uploadMsg = document.createElement('div');
            uploadMsg.style.cssText = "position: absolute; bottom: 80px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.8); padding: 10px 20px; border-radius: 20px; z-index: 1000;";
            uploadMsg.innerText = "Uploading media...";
    ...
    function openChat(chatId, title, type, participants) {
        ...
        const msgInputBox = document.querySelector('.message-input');
        if (msgInputBox) msgInputBox.style.display = 'flex';
        ...
        unsubscribeMessages = onSnapshot(q, (snapshot) => {
            ...
            snapshot.forEach(docSnap => {
                ...
                if (msg.deletedFor && msg.deletedFor.includes(uid)) return;
                ...
                if (msg.mediaUrl) {
                    if (msg.mediaType === 'video') {
                        ...
                        mediaEl.style.cssText = "max-width: 100%; border-radius: 8px; margin-top: 8px;";
                    } else {
                        mediaEl = document.createElement('img');
                        mediaEl.src = msg.mediaUrl;
                        mediaEl.style.cssText = "max-width: 100%; border-radius: 8px; margin-top: 8px; cursor: pointer;";
                        mediaEl.onclick = () => window.open(msg.mediaUrl, '_blank');
                    }
                }
                ...
                div.addEventListener('contextmenu', (e) => {
                    e.preventDefault();
                    showContextMenu(e.pageX, e.pageY, msgId, isMine, chatId);
                });
            ...
            messagesArea.innerHTML = `<div style="padding: 20px; color: #ef4444; text-align: center;">Error loading messages: ${error.message}</div>`;
        });
    }

    function showContextMenu(x, y, msgId, isMine, chatId) {
        ...
        let html = `<div class="menu-item" id="btn-del-me" style="padding: 8px 12px; cursor: pointer; color: white;">Delete for me</div>`;
        if (isMine) {
            html += `<div class="menu-item" id="btn-del-everyone" style="padding: 8px 12px; cursor: pointer; color: #ef4444;">Delete for everyone</div>`;
        }
        menu.innerHTML = html;
        ...
        document.getElementById('btn-del-me').onclick = async () => {
            const uid = getCurrentUser().uid;
            try {
                ...
                deletedFor.push(uid);
                await updateDoc(docRef, { deletedFor });
            } catch(e) { console.error(e); }
        };
        
        if (isMine) {
            document.getElementById('btn-del-everyone').onclick = async () => {
                try {
                    await updateDoc(doc(db, `chats/${chatId}/messages`, msgId), { deletedForEveryone: true });
                } catch(e) { console.error(e); }
            };
        }
    }
